...
DownloadImage("https://i.imgur.com/vd0r7AH.jpeg", "Havoc/", "Super Cute Cat")
DownloadImage("https://i.imgur.com/vd0r7AH.jpeg", "Havoc/Images/", "Reallyyyy Cute Cat")
...
if not isfolder("Havoc/Assets") then
    makefolder("Havoc/Assets")
end
if not isfolder("Havoc/Characters") then
    makefolder("Havoc/Characters")
end
if not isfile("Havoc/Characters/Tryhard.Character") then
    writefile("Havoc/Characters/Tryhard.Character", [[
...
do --[[ Functions ]]--
    ...
    do --[[ Extras ]]--  
        if isfolder("Havoc/Configs") then
            for i, v in pairs(listfiles("Havoc/Configs")) do
                local filename = v:match("^.+[\\/](.+)$")
                local nameWithoutExtension = filename:match("(.+)%..+$")
                table.insert(Havoc.Configs, nameWithoutExtension)
            end
        else
            makefolder("Havoc/Configs")
        end

        if isfolder("Havoc/Characters") then
            for i, v in pairs(listfiles("Havoc/Characters")) do
                local filename = v:match("^.+[\\/](.+)$")
                local nameWithoutExtension = filename:match("(.+)%..+$")
                table.insert(Havoc.Characters, nameWithoutExtension)
            end
        else
            makefolder("Havocisclosed/Characters")
        end
    end
    ...
    do --[[ Client ]]--
        Script.Functions.LoadAvatar = function(ConfigName)
            if not Script.Functions.ClientChecks() then return end
            local CharacterConfig = readfile(("Havoc/Characters/%s"):format(ConfigName .. ".Character"))
...
Script.Functions.CreateConnection(RunService.Heartbeat, function()
    ...
    DrawTargetName.Text = Target and "Havoc" .. Target.Name .. "" or ""
end)

Script.Functions.CreateConnection(RunService.RenderStepped, function(DeltaTime)
    if not UI.Combat.Legitbot.Enabled then return end
    if not Script.Functions.TargetExists() then return end

    local CurrentTime = tick()
    if (CurrentTime - Script.Data.LegitbotTick) < UI.Combat.Legitbot.RefreshRate then return end
    Script.Data.LegitbotTick = CurrentTime

    local TargetCharacter = Script.Targeting.Target and Script.Targeting.Target.Character
    if not TargetCharacter then return end

    local TargetPart = Script.Functions.GetTargetPart(TargetCharacter)
    if not TargetPart then return end

    local TargetPosition = TargetPart.Position
    local PredictionOffset = UI.Combat.Legitbot.Prediction and (TargetPart.Velocity * UI.Combat.Legitbot.PredictionValue) or Vector3.zero
    local AimPosition = TargetPosition + PredictionOffset

    local FinalCFrame = CFrame.lookAt(Camera.CFrame.Position, AimPosition)
    Camera.CFrame = Camera.CFrame:Lerp(FinalCFrame, UI.Combat.Legitbot.Smoothness)
end)
...
---------------------------------------[[ Library ]]---------------------------------------
local TitleString = "Havoc | Beta"


do -- [[ UI ]]
    local Window = Library:Window({Name = TitleString, PageAmmount = 7, Size = UDim2.new(0, 615, 0, 639)})
	local Watermark = Library:Watermark(TitleString)
	local KeybindList = Library:KeybindList()

    local CombatTab = Window:Page({Name = 'Target', Columns = 2})
    local AntiAimTab = Window:Page({Name = 'Anti Aim', Columns = 2})
    local MiscTab = Window:Page({Name = 'Player', Columns = 2})
    
   
    local SettingsTab = Window:Page({Name = 'Settings', Columns = 2})

	do --[[ Combat ]]--
        local TargetingSection = CombatTab:Section({Name = 'Target Aim', Side = 1})
        local SilentAimSection = CombatTab:Section({Name = 'Silent Aim      (Target Aim)', Side = 2})
        local LegitbotSection = CombatTab:Section({Name = 'Camlock   (Enable Sticky Aim)', Side = 2})
       

        local TargetVisualsSection = CombatTab:Section({Name = 'Target Visuals', Side = 1})
        local ChecksSection = CombatTab:Section({Name = 'Checks', Side = 1})
        local FieldOfViewSection = CombatTab:Section({Name = 'FOV Circle', Side = 1})
	...
	for _, Conn in pairs(getconnections(Tool.Activated)) do
		...
		for i = 1, Info.nups do
			...
			if type(val) == "number" then
				OriginalValues[Conn.Function] = OriginalValues[Conn.Function] or val

				if RapidFireEnabled then
					debug.setupvalue(Conn.Function, i, Delay)
				else
					debug.setupvalue(Conn.Function, i, OriginalValues[Conn.Function])
				end
			end
		end
	end
...
local function ApplyRapidFire(Character)
...
TargetingSection:Toggle({
	Name = "Rapid Fire",
	Flag = "Sticky Targeting",
	Callback = function(State)
		RapidFireEnabled = State

		local Character = LocalPlayer.Character
		if Character then
			ApplyRapidFire(Character)
		end

		-- Notification ONLY when enabled
		if State then
			Library:Notification(
				"Reset Character after turning off",
				5,
				Color3.fromRGB(
					math.random(0,255),
					math.random(0,255),
        ...
        do --[[ Legitbot ]]--
            ...
            LegitbotSection:Toggle({Name = 'Prediction', Flag = 'Legitbot Prediction Enabled', Callback = function(State)
                UI.Combat.Legitbot.Prediction = State
            end})

            LegitbotSection:Slider({Name = "Prediction", Min = 0, Max = 0.15, Suffix = "", Decimals = 0.0001, Flag = "Legitbot Prediction", Default = 0.095, Callback = function(Value)
                UI.Combat.Legitbot.PredictionValue = Value
            end})

            LegitbotSection:Divider()

            LegitbotSection:Slider({Name = "Smoothness", Min = 0, Max = 2, Suffix = "", Decimals = 0.01, Flag = "Legitbot Smoothness", Default = 2, Callback = function(Value)
                UI.Combat.Legitbot.Smoothness = Value
            end})

            LegitbotSection:Slider({Name = "Refresh Rate", Min = 0, Max = 1, Suffix = "", Decimals = 0.01, Flag = "Legitbot Refresh Rate", Default = 0, Callback = function(Value)
                UI.Combat.Legitbot.RefreshRate = Value
            end})
        end
        ...
        do --[[ Target Visuals ]]--
            ...
            TargetVisualsSection:Toggle({Name = 'Target Name ESP', Flag = 'Target Name', Callback = function(State)
                UI.Combat.TargetVisuals.TargetText = State
            end})

            TargetVisualsSection:Label({Name = "Color"}):Colorpicker({Name = "Text Color", Flag = "Text Color", Default = Color3.fromRGB(255, 255, 255), Callback = function(Color)
        ...
        do --[[ Field of View ]]--
            ...
            FieldOfViewSection:Slider({Name = "FOV Radius", Min = 0, Max = 1000, Decimals = 1, Suffix = "°", Flag = "Fov Radius", Default = 120, Callback = function(Value)
            ...
            FieldOfViewSection:Toggle({Name = 'Lock on Target', Flag = 'Stick Fov Target', Callback = function(State)
    ...
    do --[[ Misc ]]--
        ...
        local ESP = MiscTab:Section({Name = 'ESP', Side = 1})

      -- ESP integration using your UI library API (use inside the scope where `ESP` is defined)
-- Assumptions:
--   local ESP = MiscTab:Section({Name = 'ESP', Side = 1}) already exists
-- This file:
--   - Adds a main ESP Toggle that fully loads/unloads (creates drawings + connections / removes them)
--   - Adds two extra toggles (Tracers, Healthbar) in the same section
--   - Adds three colorpickers built via the Section:Label -> Label:Colorpicker pattern from your docs
--   - Defaults: Tracers = OFF, Healthbar = OFF
-- Paste this under/after the line where `ESP` is created.
...
-- ensure Script.Toggles table exists (many UIs expect it)
if not Script then Script = {} end
Script.Toggles = Script.Toggles or {}

-- drawing availability check
local drawing_available = pcall(function() return Drawing.new end)

-- Settings (defaults: tracers and healthbar OFF)
local Settings = {
...
local function removeDrawingSafe(d)
    if not d then return end
    pcall(function()
        if type(d.Remove) == "function" then
            d:Remove()
        else
            -- fallback: hide
...
local function cleanupDrawings(tbl)
    for _, d in pairs(tbl) do removeDrawingSafe(d) end
end

-- per-player storage
local espPlayers = {} -- [player] = { drawings = {...}, conn = RenderSteppedConnection }
...
local function createESPfor(plr)
    ...
    local lib = {
        ...
        greenhealth = NewLine(1.5, Settings.Health_Color)
    }

    -- ensure all drawings created
    ...
    conn = RunService.RenderStepped:Connect(function()
        ...
        if not hrp or not hum or not head or hum.Health <= 0 then
            for _, d in pairs(lib) do d.Visible = false end
            return
        end
        ...
        local function Size(q)
            q.PointA = Vector2.new(HumPos.X + DistanceY, HumPos.Y - DistanceY*2)
            q.PointB = Vector2.new(HumPos.X - DistanceY, HumPos.Y - DistanceY*2)
            q.PointC = Vector2.new(HumPos.X - DistanceY, HumPos.Y + DistanceY*2)
            q.PointD = Vector2.new(HumPos.X + DistanceY, HumPos.Y + DistanceY*2)
        end

        Size(lib.box)
        Size(lib.black)
        lib.box.Color = Settings.Box_Color
        lib.black.Color = black

        -- Tracer behaviour
        if Settings.Tracers then
            if Settings.Tracer_Origin == "Middle" then
                lib.tracer.From = camera.ViewportSize * 0.5
                lib.blacktracer.From = camera.ViewportSize * 0.5
            else
                lib.tracer.From = Vector2.new(camera.ViewportSize.X * 0.5, camera.ViewportSize.Y)
                lib.blacktracer.From = Vector2.new(camera.ViewportSize.X * 0.5, camera.ViewportSize.Y)
            end
            if Settings.Tracer_FollowMouse then
                lib.tracer.From = Vector2.new(mouse.X, mouse.Y + 36)
                lib.blacktracer.From = Vector2.new(mouse.X, mouse.Y + 36)
            end
            lib.tracer.To = Vector2.new(HumPos.X, HumPos.Y + DistanceY*2)
            lib.blacktracer.To = Vector2.new(HumPos.X, HumPos.Y + DistanceY*2)

            lib.tracer.Color = Settings.Tracer_Color
            lib.tracer.Thickness = Settings.Tracer_Thickness
            lib.blacktracer.Thickness = math.max(1, Settings.Tracer_Thickness * 2)
            lib.blacktracer.Color = black
        ...
        local healthoffset = hum.Health / math.max(1, hum.MaxHealth) * d

        lib.greenhealth.From = Vector2.new(HumPos.X - DistanceY - 4, HumPos.Y + DistanceY*2)
        lib.greenhealth.To = Vector2.new(HumPos.X - DistanceY - 4, HumPos.Y + DistanceY*2 - healthoffset)

        lib.healthbar.From = Vector2.new(HumPos.X - DistanceY - 4, HumPos.Y + DistanceY*2)
        lib.healthbar.To = Vector2.new(HumPos.X - DistanceY - 4, HumPos.Y - DistanceY*2)

        lib.greenhealth.Color = Color3.fromRGB(255,0,0):lerp(Settings.Health_Color, hum.Health / math.max(1, hum.MaxHealth))
        lib.healthbar.Color = black

        lib.healthbar.Visible = Settings.Healthbar
        lib.greenhealth.Visible = Settings.Healthbar

        lib.box.Visible = true
        lib.black.Visible = true
    end)

    espPlayers[plr] = { drawings = lib, conn = conn }
end

local function removeESPfor(plr)
    local data = espPlayers[plr]
    if not data then return end
    if data.conn and data.conn.Connected then pcall(function() data.conn:Disconnect() end) end
    cleanupDrawings(data.drawings)
    espPlayers[plr] = nil
end
...
local function enableESP()
    if not drawing_available then
        warn("[ESP] Drawing API unavailable. ESP will not run.")
        return
    end
    if espEnabled then return end
    espEnabled = true
    Script.Toggles.ESP = true

    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= player then createESPfor(plr) end
    end
...
local function disableESP()
    ...
    espEnabled = false
    Script.Toggles.ESP = false

    if playerAddedConn and playerAddedConn.Connected then pcall(function() playerAddedConn:Disconnect() end) end
    if playerRemovingConn and playerRemovingConn.Connected then pcall(function() playerRemovingConn:Disconnect() end) end
    playerAddedConn = nil
...
-- Recreate ESP for respawned players
...
-- ===== UI wiring using the documented API =====
-- Main ESP toggle (uses Section:Toggle)
ESP:Toggle({
    Name = "ESP",
    Default = false,
    Flag = "ESP_Main",
...
-- Colorpickers: using Section:Label -> Label:Colorpicker (per your docs)
-- Box color
local boxLabel = ESP:Label({ Name = "Box Color" })
if boxLabel and type(boxLabel.Colorpicker) == "function" then
    boxLabel:Colorpicker({
        Name = "Box Color",
        Default = Settings.Box_Color,
        Alpha = false,
        Flag = "ESP_BoxColor",
        Callback = function(col)
            Settings.Box_Color = col
        end
    })
else
    -- Fallback: if your library supports Section:Colorpicker directly, try that
    if ESP.Colorpicker then
        ESP:Colorpicker({
            Name = "Box Color",
            Default = Settings.Box_Color,
            Alpha = false,
            Flag = "ESP_BoxColor",
            Callback = function(col) Settings.Box_Color = col end
        })
    end
end

-- Tracer color
local tracerLabel = ESP:Label({ Name = "Tracer Color" })
if tracerLabel and type(tracerLabel.Colorpicker) == "function" then
    tracerLabel:Colorpicker({
        Name = "Tracer Color",
        Default = Settings.Tracer_Color,
        Alpha = false,
        Flag = "ESP_TracerColor",
        Callback = function(col)
            Settings.Tracer_Color = col
        end
    })
else
    if ESP.Colorpicker then
        ESP:Colorpicker({
            ...
            Alpha = false,
            Flag = "ESP_TracerColor",
            Callback = function(col) Settings.Tracer_Color = col end
        })
    end
end

-- Health color
local healthLabel = ESP:Label({ Name = "Health Color" })
if healthLabel and type(healthLabel.Colorpicker) == "function" then
    healthLabel:Colorpicker({
        Name = "Health Color",
        Default = Settings.Health_Color,
        Alpha = false,
        Flag = "ESP_HealthColor",
        Callback = function(col)
            Settings.Health_Color = col
        end
    })
else
    if ESP.Colorpicker then
        ESP:Colorpicker({
            ...
            Alpha = false,
            Flag = "ESP_HealthColor",
            Callback = function(col) Settings.Health_Color = col end
        })
    end
end

-- ensure ESP is turned off when the script unloads (best-effort)
if syn and syn.unload_hook then
    syn.unload_hook(function() disableESP() end)
end

-- Done. The UI elements are added to your `ESP` section and will control the ESP system.

        local AvatarLoader = SettingsTab:Section({Name = 'Avatars', Side = 1})
        ...
        do -- movement
            ...
            Movement:Slider({Name = "Fly Speed", Min = 10, Max = 2000, Suffix = "", Decimals = 1, Flag = "Fly Speed", Default = 250, Callback = function(Value)
            ...
            end}):Keybind({Name = 'CFrame Speed Bind', Default = Enum.KeyCode.V, Flag = "CFrame Keybind", Mode = "Toggle", Callback = function(Key)
        ...
        do -- client stuff
            ...
            end}):Keybind({Name = '', Default = Enum.KeyCode.T, Flag = "Noclip Keybind", Mode = "Toggle", Callback = function(State)
                if not Library.Flags['Noclip'] then return end
                if not Script.Functions.ClientChecks() then return end

                if State then
                    for _, Part in pairs(Script.Client.Character:GetChildren()) do
            ...
            ClientStuff:Toggle({Name = 'Face Backwards', Flag = 'Face Backwards', Callback = function(State)
                UI.Misc.Client.FaceBackwards = State
            end})

            ClientStuff:Toggle({Name = 'Anti Void', Flag = 'Anti Void', Callback = function(State)
                if State then
                    workspace.FallenPartsDestroyHeight = 0/0
                else
                    workspace.FallenPartsDestroyHeight = Script.Data.DestroyHeight
                end
            end})
            ...
            ClientStuff:Dropdown({Name = "Animation", Flag = "Animation", Default = "Floss", Items = {"Floss", "More soon"}, Callback = function(Option)
        ...
        do -- avatar loader
            local AvatarListbox = AvatarLoader:Listbox({Flag = "Avatar Name", Options = Havoc.Characters, Callback = function(Option)
            ...
            AvatarLoader:Button({Name = "Refresh List", Callback = function()
				if isfolder("Havoc/Characters") then
					for I, V in pairs(listfiles("Havoc/Characters")) do
						...
						local NameWithoutExtension = FileName:match("(.+)%..+$")
						local Found = false
						for J, ExistingConfig in pairs(Havoc.Characters) do
				...
				AvatarListbox:Refresh(Havoc.Characters)
			end})
	...
	do -- Settings Tab
		...
		local ConfigsSection = SettingsTab:Section({Name = "Configurations", Side = 1})
		...
		SettingsSection:Label({Name = "Menu Keybind", Alignment = "Left"}):Keybind({Name = "Menu Keybind", Flag = "Menu Keybind", Default = Enum.KeyCode.F4, Mode = "Toggle", Callback = function(Value)
        ...
        SettingsSection:Button({Name = "Status", Callback = function()
			Library:Notification("Havoc is currently undetected and safe to use.", 5, Color3.fromRGB(math.random(0, 255), math.random(0, 255), math.random(0, 255)))
		end})
